...
if nav_target in content and 'label="TK PANEN"' not in content:
    ...
    print("Added TK PANEN nav item to laporan_produksi.html")
...
if app_target in content and 'function TKPanenView()' not in content:
    content = content.replace(app_target, tk_view + '\n\n    ' + app_target)
    print("Added TKPanenView component to laporan_produksi.html")

# 3. View Render
app_render_target = '{activeNav === \'validasi\' ? ('
app_render_replacement = '''{activeNav === 'tk-panen' ? (
                  <TKPanenView />
                ) : activeNav === 'validasi' ? ('''

if app_render_target in content and 'activeNav === \'tk-panen\'' not in content:
    content = content.replace(app_render_target, app_render_replacement)
    print("Added view render for tk-panen in laporan_produksi.html")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("SUCCESSFULLY PATCHED laporan_produksi.html!")
