...
|---|---|---|
| **2026-08-19** | **Pengaktifan Edit Kebutuhan TK & Luasan Lahan pada Modal Edit Kebun** | Mengubah input **Kebutuhan TK Panen** (`req_tk`) dan **Luasan Lahan (Ha)** (`luasan`) pada modal edit data kebun agar dapat diisi oleh akun regional dan admin. Mengintegrasikan sinkronisasi datanya secara real-time di UI (tabel & KPI badges otomatis ter-update instan) serta menyinkronkan penyimpanan ke database JSON dan Live Supabase via API `/api/kebunTK`. |
| **2026-08-19** | **Penguncian Input Modal, Halaman Kloning Publik & Koreksi Grahadura** | Mengunci otomatis (*auto-populate & lock*) isian **CRO** dan **Region** pada modal "Tambah Kebun Baru" untuk akun regional baik di admin (`laporan_produksi.html`) maupun regional (`login.html`), sedangkan akun Admin tetap bebas mengedit. Membuat halaman kloning publik [rekap_tk_panen.html](file:///d:/AGRINAS%20PALMA%20NUSANTARA/AGRIPAM/rekap_tk_panen.html) untuk bot screenshot tanpa login yang mendukung *Fit to Screen*, *Fit to Paper* (print styling khusus A4), dan default *Light Mode* (warna latar terang). Memperbarui luasan kebun **Grahadura Leidongprim/Bakrie Group** (ID: 801) dari `0` menjadi `3260.34` ha di skrip python generator, database lokal JSON, dan live Supabase. |
